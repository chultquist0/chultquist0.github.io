...
const GuessTheImage = () => {
  ...
  const handleNextImage = () => {
    ...
    setUserGuesses(prev => [...prev, { score: sliderValue, class: images[currentImageIndex].class }]);
  ...
  const calculateAUCScore = (classCScores, classBScores) => {
    ...
    const labels = filteredClassCScores.map(score => ({ score, label: 1 })).concat(
  ...
  return (
    <div className="p-4 max-w-lg mx-auto border border-gray-300 rounded shadow-lg">
      {!finished ? (
        <>
          <h2 className="text-2xl font-semibold mb-4">Test your B-Tagging!</h2>
          <img
            src={images[currentImageIndex].src}
            className="w-64 h-auto rounded mb-4"
          />
          <p className="text-md mb-2"> {currentImageIndex + 1} / {images.length}</p>
          <div className="mb-4">
            <label className="block mb-2"> Assign this jet a b-tagging score:</label>
            <input
              type="range"
              min="1"
              max="5"
              ...
              className="w-full"
              style={{ backgroundSize: '100% 100%', backgroundImage: 'linear-gradient(to right, #00C49F 0%, #00C49F ' + (sliderValue * 20) + '%, #ddd ' + (sliderValue * 20) + '%, #ddd 100%)' }}
