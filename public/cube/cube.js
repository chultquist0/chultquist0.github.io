...
const faceRotations = {
  front: { x: -1.1, y: 0, z: 0 },
  back: { x: -0.9, y: Math.PI, z: 0 },
  left: { x: -1, y: 1.5, z: 0 },
  right: { x: -0.9, y: -1.5, z: 0 },
  top: { x: 0.2, y: 0, z: 0 },
...
function get_selected_clue() {
  for (let i = 0; i < gridSize; i++) {
    ...
    const hint_number = cubenumber[3 - cube.grid_pos.y][cube.grid_pos.z][cube.grid_pos.x]
      ...
      for (let z = 0; z < cubenumber[x][y].length; z++) {
        if (cubenumber[x][y][z] === clueNumber) {
          ...
          return group.children.find(
            (cube) => cube.grid_pos.x === z && cube.grid_pos.y === 3 - x && cube.grid_pos.z === y
          )
        }
      }
    }
...
function resetViewAndHighlight(clueId) {
  ...
  if (seq_dir == SEQDIR.x) selected_seq = selected_seq.sort((a, b) => a.grid_pos.x - b.grid_pos.x)
  if (seq_dir == SEQDIR.z) selected_seq = selected_seq.sort((a, b) => a.grid_pos.z - b.grid_pos.z)
  if (seq_dir == SEQDIR.y) selected_seq = selected_seq.sort((a, b) => b.grid_pos.y - a.grid_pos.y)
  ...
  for (let y = gridSize - 1; y >= 0; y--) {
    for (let z = 0; z < gridSize; z++) {
      if (
        ...
        labelMesh.position.set(cubeMesh.position.x, cubeMesh.position.y, cubeMesh.position.z)
        ...
        let labelTexture = createNumberTexture(cubenumber[3 - y][z][x], 512, true)
  ...
  for (let x = 0; x < answer.length; x++) {
    for (let y = 0; y < answer[x].length; y++) {
      for (let z = 0; z < answer[x][y].length; z++) {
        const cube = group.children.find(
          (cube) => cube.grid_pos.x === z && cube.grid_pos.y === 3 - x && cube.grid_pos.z === y
        )
...
renderer.domElement.addEventListener('click', (event) => {
  ...
  for (pot_cube of group.children) {
    if (seq_dir == 0) {
      if (pot_cube.grid_pos.y == cube.grid_pos.y && pot_cube.grid_pos.z == cube.grid_pos.z) {
        selected_seq.push(pot_cube)
      }
    } else if (seq_dir == 1) {
      if (pot_cube.grid_pos.y == cube.grid_pos.y && pot_cube.grid_pos.x == cube.grid_pos.x) {
        selected_seq.push(pot_cube)
      }
    } else if (seq_dir == 2) {
      if (pot_cube.grid_pos.z == cube.grid_pos.z && pot_cube.grid_pos.x == cube.grid_pos.x) {
        selected_seq.push(pot_cube)
      }
    }
  }
